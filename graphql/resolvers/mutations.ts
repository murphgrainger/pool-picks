...
export const Mutation = {
  ...
  createPoolInvite: async (_: any, args: any, context: any) => {
    try {
      const poolInvite = await prisma.poolInvite.create({
        data: {
          ...
          pool: {
            connect: { id: args.pool_id },
          },
        },
      });
      return poolInvite;
    } catch (error: any) {
      throw new ApolloError(`Could not create pool invite: ${error.message}`);
    }
  },
  ...
  updatePoolMemberUsername: async (_: any, args: any, context: any) => {
    try {
      const response = await prisma.poolMember.update({
        where: { id: parseInt(args.id) },
  ...
  updateTournament: async (_: any, args: any, context: any) => {
    try {
      ...
      throw new Error(`Could not update tournament: ${error.message}`);
    }
  },
  updatePool: async (_: any, args: any, context: any) => {
    try {
      const response = await prisma.pool.update({
        where: { id: parseInt(args.id) },
        data: { status: args.status },
      });
      return response;
    } catch (error: any) {
      throw new Error(`Could not update pool: ${error.message}`);
    }
  },
};
