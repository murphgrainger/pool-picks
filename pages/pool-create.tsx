...
const PoolCreate = () => {
  ...
  const [createPool, { data, loading, error }] =
    useMutation(CreatePoolMutation);
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  // Set the tournament_id from URL query parameter if it exists
  useEffect(() => {
    if (tournament_id) {
      setValue("tournament_id", tournament_id as string);
    }
  }, [tournament_id, setValue]);

  const onSubmit: SubmitHandler<FormValues> = async (data) => {
    const { name, amount_entry, tournament_id } = data;
    const variables = {
      name,
      amount_entry: parseInt(amount_entry, 10),
      tournament_id: parseInt(tournament_id, 10),
    };
    try {
      const result = await toast.promise(createPool({ variables }), {
      ...
      if (result.data?.createPool?.id) {
        router.push(`/pool/${result.data.createPool.id}`);
      }
